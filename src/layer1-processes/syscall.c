...
#if USE_QL_SCHED
#include "q_learning/qlearning_sched.h"   /* shelved: lives in non-cs452/ */
#endif
#include "vmm.h"
#include "locks.h"
...
extern void recieve_helper(int receiver_tid);
...
void block(int pid, uint8_t priority){
	if (pid < 1 || pid > NUMPROCS) return;   /* defensive: never index out of range */
...
void ExceptionASYNC(uint64_t esr_el1){
	...
	*/
	...
	switch (interruptid) {
		case CLOCKINTID:
			// ARM generic timer tick. Re-arm the timer and wake the clock notifier.
			gentimer_arm_ms(10);
		...
		case APU3_DONE_EVENT:
			/* Core N worker → SGI to CPU0. Unblock APU{N}_server in AwaitEvent,
			 * or queue the completion if the server has not registered yet. */
...
void handlerExceptionHelper(uint64_t esr_el1)
{
	...
	int p = PID - 1;
	if ((esr_el1 >> 24) != 86) {
		/* Non-SVC synchronous exception (e.g. trapped system register access).
		 * Re-add the faulting task so it isn't silently lost from the scheduler. */
		scrSchedule(PID, PROCS[p].priority);
		return;
	}
	if (esr_el1 >> 24 == 86) { // an svc call has occured!
		...
		case 6: // recieve blocks
			...
			#endif
			//PROCS[p].registervalues[0] = 
			recieve_helper((int)PROCS[p].pid);
		...
		case 14: // GetProcessSharedMem - return base pointer of this process's shared memory
			...
			break;
		case 20: // CreateInProcess - spawn EL0 thread in a specific process container
			scrSchedule(PID, PROCS[p].priority);
...
int KernelCreate(uint8_t priority, void (*function)(), int parent)
{
	...
	if (PROCS[p].pcpointer == NULL) {
		...
		PROCS[p].process_id = (parent > 0) ? PROCS[parent - 1].process_id : 0;
...
int KernelCreateInProcess(uint8_t priority, void (*function)(), int parent, int process_id)
{
	...
	/* Find a free PCB slot, bounded to [0, NUMPROCS). A dead/unused task has
	 * pcpointer == NULL (Kill clears it), so freed slots are reused. NEVER walk
	 * past NUMPROCS: an out-of-range pid later indexes BLOCKED_LIST/PROCS out of
	 * bounds, and BLOCKED_LIST[NUMPROCS] aliases READY_HEAP (corruption). */
	int p = -1;
	for (int i = 0; i < NUMPROCS; i++) {
...
void Kill(int p) // p is the position of the process in the PROCS array
{
	...
	PROCS[p].pcpointer = NULL;   // marks the slot free for reuse (see KernelCreateInProcess)
}
