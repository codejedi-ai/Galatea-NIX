...
// BCM2711 (Raspberry Pi 4): GPIO and PL011 UART via mmio_config.h
...
void uart_putc(size_t line, unsigned char c) {
  // Pi 4 PL011: write directly (TX FIFO status via FR)
...
void uart_puts(size_t line, const char* buf) {
  if (line == CONSOLE && console_route_active()) {
    int len = 0;
    while (buf[len]) len++;
    console_route_write(buf, len);
    return;
  }
  while (*buf) {
    uart_putc(line, *buf);
...
static int uart_format_to_buf(char *out, int cap, char *fmt, va_list va)
{
	char bf[12];
	char ch;
	int pos = 0;
	...
	while ((ch = *(fmt++))) {
		if (ch != '%') {
			...
			case 'u':
				ui2a(va_arg(va, unsigned int), 10, bf);
				for (int i = 0; bf[i] && pos + 1 < cap; i++) out[pos++] = bf[i];
				break;
			case 'd':
				i2a(va_arg(va, int), bf);
				for (int i = 0; bf[i] && pos + 1 < cap; i++) out[pos++] = bf[i];
			...
			case 's': {
				...
				for (int i = 0; s[i] && pos + 1 < cap; i++) out[pos++] = s[i];
				break;
			}
			case 'b':
				ui2a(va_arg(va, unsigned int), 2, bf);
				...
				break;
			case 'c':
				if (pos + 1 < cap) out[pos++] = (char)va_arg(va, int);
				break;
			case '%':
...
void uart_printf( size_t line, char *fmt, ... ) {
	...
	if (line == CONSOLE && console_route_active()) {
		char buf[512];
		int n = uart_format_to_buf(buf, (int)sizeof(buf), fmt, va);
		if (n > 0) console_route_write(buf, n);
	} else {
